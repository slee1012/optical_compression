...
class OpticalPropagation:
    ...
    @staticmethod
    def propagate(field, distance, wavelength, pixel_pitch, method=None, padding_factor=1.0):
        if method == 'angular' or method == 'angular_spectrum':
            return OpticalPropagation.angular_spectrum(field, distance, wavelength, pixel_pitch, padding_factor=padding_factor)
        else:
